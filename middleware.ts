...
export async function middleware(request: NextRequest) {
  ...
  const isPublicApi =
    ...
    pathname === '/api/health' ||
    pathname === '/api/debug/health-check';
  const isStaticAsset =
    pathname.startsWith('/_next') || pathname.startsWith('/assets') || pathname.includes('favicon.ico');
  ...
  try {
    ...
    // 5. AUTH GUARD
    if (!user) {
      const isAppPath = ['/dashboard', '/student', '/trainer', '/admin'].some(p =>
    ...
    if (pathname.startsWith('/admin') && role !== 'admin') {
      return NextResponse.redirect(
        new URL(role === 'lead_trainer' ? '/dashboard' : '/student', request.url)
      );
    }
